...
class CartController extends Controller
{
    public function index()
    {
        ...
        if (Auth::guard('customer')->check()) {
            ...
            if ($cart && !$cart->items->isEmpty() && !$primaryAddress) {
            ...
            if (!empty($sessionCart)) {
                ...
                $cart = (object) [
                    'items' => $items,
                    'total' => $items->sum(fn($item) => $item->qty * $item->harga),
                ];
    ...
    // GET SHIPPING RATE
    // ============================================================
    public function getShippingRate(Request $request, FedexService $fedexService)
    {
        try {
            ...
            if (!$addressId || !$weight) {
                return response()->json([
                    'success' => false,
                    'error'   => 'address_id dan weight diperlukan.',
                ], 400);
            }

            // ---------------------------------------------------------
            // 2. Ambil alamat + relasi country sekaligus
            // ---------------------------------------------------------
            $address = UserAddress::with('country')->find($addressId);
            ...
            // ---------------------------------------------------------
            // 3. Ambil zip_code dan country code
            // ---------------------------------------------------------
            ...
            Log::info('CartController@getShippingRate: Address data', [
                'address_id'       => $addressId,
                'zip_code'         => $zip,
                'kode_iso'         => $address->kode_iso ?? null,
                'country_iso'      => $address->country?->iso2 ?? null,
                'resolved_country' => $country,
                'weight'           => $weight,
            ]);

            // ---------------------------------------------------------
            // 4. Validasi zip dan country
            // ---------------------------------------------------------
            if (empty($zip)) {
                return response()->json([
                    'success' => false,
                    'error'   => 'Zip code pada alamat tidak boleh kosong. Silakan update alamat Anda.',
                ], 422);
            }
            ...
            if (empty($country) || strlen($country) !== 2) {
                return response()->json([
                    'success' => false,
                    'error'   => 'Kode negara pada alamat tidak valid. Pastikan formatnya 2 huruf (contoh: US, SG, AU).',
                ], 422);
            }

            // ---------------------------------------------------------
            // 5. Ambil cart untuk build items dan total value
            // ---------------------------------------------------------
            $customer   = Auth::guard('customer')->user();
            ...
            $totalValue = $cart
                ? (float) $cart->items->sum(fn($item) => $item->harga * $item->qty)
                : 1;

            // Build items dengan dimensions (sama seperti FedExController)
            $items = [];
            if ($cart) {
            ...
            // ---------------------------------------------------------
            // 6. Bangun data untuk FedexService
            // ---------------------------------------------------------
            $rateData = [
            ...
            Log::info('CartController@getShippingRate: Calling FedexService->getRates()', $rateData);

            // ---------------------------------------------------------
            // 7. Panggil FedexService dan format hasilnya
            //    (FIX: hanya 1x panggilan, response dikembalikan dalam
            //    format {success, rates} yang konsisten dengan endpoint lain)
            // ---------------------------------------------------------
            try {
                $rates = $fedexService->getRates($rateData);
            } catch (\Throwable $e) {
                Log::error('CartController@getShippingRate: FedexService exception', [
                ...
                return response()->json([
                    'success' => false,
                    'error'   => 'Gagal mengambil shipping rate dari FedEx. Silakan coba lagi.',
                    'message' => $e->getMessage(),
                ], 500);
            }

            Log::info('CartController@getShippingRate: Rates formatted', [
                'count' => count($rates),
                'rates' => $rates,
            ]);
            ...
            Log::error('CartController@getShippingRate: Exception', [
                'message' => $e->getMessage(),
                'file'    => $e->getFile(),
                'line'    => $e->getLine(),
                'trace'   => $e->getTraceAsString(),
            ...
            return response()->json([
                'success' => false,
                'error'   => 'Gagal mengambil shipping rate. Silakan coba lagi.',
                'message' => $e->getMessage(),
            ], 500);
        }
    }
    ...
    // ============================================================
    // SET ADDRESS — reset shipping (dipanggil saat user ganti alamat)
    // ============================================================
    ...
    public function saveShipping(Request $request)
    {
        ...
        try {
            ...
            Log::info('CartController@saveShipping: shipping saved', [
                'cart_id'          => $cart->id,
                'shipping_service' => $request->shipping_service,
                'shipping_cost'    => $request->shipping_cost,
            ]);
            ...
            $subtotal = $cart->items->sum(fn($item) => $item->harga * $item->qty);
            $total    = $subtotal + (float) $request->shipping_cost;
            ...
            Log::error('CartController@saveShipping failed', [
                'error'   => $e->getMessage(),
                'cart_id' => $cart->id,
            ]);
    ...
    // ============================================================
    // SAVE ADDRESS (alias, tanpa reset shipping)
    // ============================================================
    ...
    // ============================================================
    // SET SHIPPING (untuk ID locale / Lion Parcel)
    // ============================================================
    ...
    // ============================================================
    // SAVE SHIPPING DETAILS (backward compatibility)
    // ============================================================
    ...
    public function updateShippingSelection(Request $request)
    {
        ...
        if (!is_null($request->shipping_service) && !is_null($request->shipping_cost)) {
            $cart->shipping_service = $request->shipping_service;
            $cart->shipping_cost    = $request->shipping_cost;
            $cart->save();

            Log::info('CartController@updateShippingSelection: saved', [
                'cart_id'          => $cart->id,
                'shipping_service' => $request->shipping_service,
                'shipping_cost'    => $request->shipping_cost,
            ]);
    ...
    {
        Log::info('EN Checkout hit', ['user_id' => Auth::guard('customer')->id()]);
        ...
        try {
            ...
            Log::info('Cart state at checkout', [
                'cart_id'          => $cart?->id,
    ...
    public function store(Request $request)
    {
        ...
        $price   = request()->segment(1) == 'en'
            ? (float) ($variant->harga ?? 0)
            : (float) ($variant->hargi ?? 0);
    ...
    public function storeLion(Request $request)
    {
        ...
        try {
            ...
            $itemAttributes = ['idcart' => $cart->id, 'idproduk' => $variant->id_produk];
            if ($variant->id_jenis)  $itemAttributes['id_jenis']  = $variant->id_jenis;
            if ($variant->id_ukuran) $itemAttributes['id_ukuran'] = $variant->id_ukuran;
    ...
    public function checkoutLionParcel(Request $request)
    {
        ...
        try {
            ...
            DB::rollBack();
            Log::channel('lionparcel')->error('CHECKOUT_ERROR', ['error' => $e->getMessage()]);
    ...
    public function updateQuantity(Request $request, $item_id)
    {
        ...
        if (Auth::guard('customer')->check()) {
            ...
            return response()->json([
                ...
                'cart_total_gross_weight' => $cart->total_gross_weight ?? 0,
            ]);
        } else {
            $cart = session('cart', []);
    ...
    public function destroy($itemId)
    {
        try {
            ...
            return redirect()->route('en.frontend.cart.index')->with('success', 'Product removed from cart.');
        } catch (\Exception $e) {
    ...
    private function prepareLionParcelPayload(Cart $cart, UserAddress $receiverAddress, $idtransaksi, $subtotal)
    {
        ...
        $items = $cart->items->map(function ($item) {
            ...
            return [
                ...
                'stt_piece_gross_weight' => (float) $item->gros,
            ];
        })->toArray();
