...
class ShipController extends Controller
{
    ...
    public function createShipment(Request $request, FedexService $fedexService)
    {
        ...
        try {
            ...
            // 3. Pre-shipment upload PDF ke FedEx → dapat docId
            $docId = null;
            try {
                ...
                $docId = $uploadResponse['output']['meta']['docId'] ?? null;

                Log::info('FedEx pre-shipment upload success.', [
                    'docId' => $docId,
                ]);
            } catch (\Throwable $uploadEx) {
                Log::warning('FedEx pre-shipment upload failed (non-critical): ' . $uploadEx->getMessage());
            }

            // 4. Create FedEx Shipment — sertakan docId jika ada
            $shipmentResult = $fedexService->createShipment($transaksi, $docId);
            $trackingNumber = $shipmentResult['trackingNumber'];

            // 5. Simpan shipment ke DB
            ...
            // 6. Update commercial invoice dengan shipment_id dan awb_number
            ...
            $outputMeta = $shipmentResult['output']['meta'] ?? null;

            FedexTradeDocument::create([
                'shipment_id'                 => $fedexShipment->id,
                'fedex_commercial_invoice_id' => $commercialInvoice->id,
                'document_type'               => $outputMeta['documentType'] ?? 'CO',
